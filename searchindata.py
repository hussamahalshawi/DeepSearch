...
class SearchInData:
    ...
    def search1(self, paragraph):
        for key, value in paragraph.items():
            self.score_output1_search[key] = (0, "")
            for text in self.text1:
                if re.search(text, value):
                    self.score_output1_search[key] = (self.score_output1_search[key][0] + 1, value)
                if re.search(self.text2, value):
                    self.score_output1_search[key] = (self.score_output1_search[key][0] + 1, value)
                if re.search(self.text3, value):
                    self.score_output1_search[key] = (self.score_output1_search[key][0] + 1, value)

        return self.score_output1_search
